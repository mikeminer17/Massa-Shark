...
if [ "$language" = "RU" ]; then
	...
	t_ni8="Входящих підключень:   ${C_LGn}%d${RES}"
	t_ni9="Виходящих підключень:  ${C_LGn}%d${RES}\n\n"
	t_ni10="   Кошельки"
	...
	t_wi5="Зареєстрований\nдля стейкінга:   ${C_LGn}так${RES}"
	t_wi6="Зареєстрований\nдля стейкінга:   ${C_R}ні${RES}"
	t_wi7="Баланс:          ${C_LGn}%.2f${RES}"
	t_wi8="ROLL всього:   ${C_LGn}%d${RES}"
	t_wi9="Активні ROLL: ${C_LGn}%d${RES}"
	
	
	t_br1="${C_R}Баланс менше 100 токенів${RES}"
	t_br2="Куплено ${C_LGn}%d${RES} ROLL"
	t_br3="Введіть кількість ROLL (максимально ${C_LGn}%d${RES}): "
	t_br4="${C_R}Недостатньо токенів для покупки${RES}"
	
	
	t_rpk="${C_R}Не вдалось зареєструвати ключ для стейкінга${RES}"
	
	
	t_ctrp1="${C_LGn}Введіть свій Discord ID (з бота):${RES} "
	t_ctrp2="\nВідправте Discord боту:\n${C_LGn}%s${RES}\n"
	
	
	t_done="${C_LGn}Готово!${RES}"
	t_err="${C_R}Не існує дії!${RES}"
	t_err_nwn="\n${C_R}Нода не працює!${RES}\nПодивитись лог: ${C_LGn}massa_log${RES}\n"
#elif [ "$language" = ".." ]; then
else
	t_ni1="\nNode ID:                ${C_LGn}%s${RES}"
